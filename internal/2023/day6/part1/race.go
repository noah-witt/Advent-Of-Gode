...
func getNumbersFromStr(section string) []int {
...
func ParseStringInput(input string) int {
	...
	times := getNumbersFromStr(timeLineTimes)
	distances := getNumbersFromStr(distanceLineDistances)
	if len(times) != len(distances) {
		panic("Must get same number of times and distances")
	}
	races := []Race{}
	for i, time := range times {
		distance := distances[i]
		// they must complete distance+1
		// this only matters if the roots are exactly integers
		races = append(races, Race{recordMM: int64(distance + 1), raceTimeMS: int64(time)})
	}

	result := ComputeProductOfRaceWins(races)
	print(result)
	return int(result)
}
